...
address:
  ...
  - code: TPM
    address: Te Pūnaha Matatini, A New Zealand Centre of Research Excellence, Private Bag 92019, Auckland 1142, New Zealand
...
Number of distinct components -- distinct/disconnected subgraphs
Number of communities (bounded by number of components)
Centrality measures: different types. 
 - Degree centrality -- measure how "important" is a node. Tells you how "ubiquitous"/"social" a soil series -- ie high degree centrailyty, that's a soil series that co-coccur with many others
 - Eigenvector centrality: how well connected are the well connected nodes to other well connected nodes (PageRank): if high the soil series that you are connected to are also highly connected. Socialbale soils connected to ither sociabale soils.
 - Betweeness centrality: how many paths go through a node. Soil series that bridges different communities. That's soils occurring on boundaries of communities? Suggest they can adapt to different soil forming factors combination? Reveal soil that have ubiquitous soil formation processes? 

--> Try and plot spatial map based on centrality

Something from Dion and Emily's paper to simplify and wordsmith:
The centrality measures used here were chosen to cover a range different types of measures while keeping the range of measures manageable. Specifically, we consider the following (from the igraph package [5]):
Node Strength An equivalent of node degree that accounts for the differing edge weights in a network. The strength of a node is simply the sum of the weights of edges connected to that node.
PageRank A popular variant of eigenvector centrality. Eigenvector-based mea- sures consider not only the strength of a node but also how well connected a node is to other nodes with high node strength.
Kleinburg Centrality A generalisation of eigenvector centrality. Nodes are imbued with two attributes: Authority—how much information/influence is held by the node; and Hubness—how well a node connects to nodes with high authority. If A is the adjacency matrix of a network, the hub score of nodes is calculated as the principle eigenvector of AAT .
Authority Relatedtohubcentrality,theauthoritycentralityofverticesiscalculated as the principle eigenvector of AT A.
Diversity The scaled Shannon entropy of the edge weights of a node. Given the context of IO networks it is worth noting that the diversity measure used here is a version of the species diversity measure commonly used in ecology to quantify the diversity of a habitat [9], not the measure of diversity sometimes used in economic geography and popularised by Hausmann and Hidalgo in [8] which is simply the node degree of a binary matrix that measures whether a region is strongly associated with particular products or exports.
Betweenness The number of weighted shortest paths that pass through a node, given all possible paths between pairs of nodes in the network.
Closeness The number of steps required to reach every other node in a network from a given node following edge-weighted paths.
...
This similarity matrix was then used to build the adjacency graph of the soil survey, which is shown Figure \ref{fig:plot_graph_combined}. 
The resulting visualisation shows a main network, surrounded by 8 other, much smaller networks. 
This shows that... 

```{r plot_graph_combined, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:plot_graph_combined}Graph representation of the soil series co-occurence. ", fig.width=12, fig.height=8}
# cowplot::plot_grid(p_g1, p_g2, p_g3, nrow = 1, labels = "auto", align = "h", label_x = 0.5, label_y = 0)
p_g1
```

Figure \ref{fig:plot_graph_annotated} shows the communities idemtified from the graph structure, and how these cluster the different soil series present in the soil survey area.  
Ultimately, 16 different communities were delineated by the algorithm. 

```{r plot_graph_annotated, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:plot_graph_annotated}Correspondance between identified communities from the graph structure and soil series.", fig.width=12, fig.height=8}
# cowplot::plot_grid(p_g1, p_g2, p_g3, nrow = 1, labels = "auto", align = "h", label_x = 0.5, label_y = 0)
p_g3
```
...
```{r graph_metrics, echo=FALSE, message=FALSE, warning=FALSE}
# Betweeness
...
```{r page_rank, echo=FALSE, message=FALSE, warning=FALSE}
nt <- V(g)$notes %>% unique
names(nt) <- nt
...
dd <- plyr::ldply(
  nt, 
  function(i) {
    res <- page.rank(igraph::induced_subgraph(g, which(V(g)$notes == i)))
    d <- data.frame(series = names(res$vector), pr = res$vector)
    d$series  <- with(d, reorder(series, pr)) 
    d
  }
) 

# p_pr <- ggplot(dd) + 
#   geom_bar(aes(x = series, y = pr), stat = "identity") + 
...
```{r plot_graph_4, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:plot_graph_4}Expert interpretation of the communities.", fig.width=10, fig.height=8}
print(p_g4)
```
