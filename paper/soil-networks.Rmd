...
Key Points
- soil series are convenient containers / labels for soil property and condition functions + covariance structure
...
In this study, the soil series that co-occur in at least one map unit are referred to as being "soil siblings". 
On the soil survey data snippet provided Table \ref{tab:sim_mat_1}, soil series \textit{Amador}, \textit{Gillender}, \textit{Ranchoseco}, \textit{Vleck}, \textit{Pardee}, and \textit{Peters} are all siblings, since they co-occur in map unit "1612048". 
However, \textit{Vleck} and \textit{Miltonhills} are \emph{not} siblings -- at least on that subset of data -- since they do not co-occur in any of the two map units presented. 
