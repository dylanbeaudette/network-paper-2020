...
mu_sf <- st_read('../data/graph-and-mu-polygons.shp')
...
# mu_sf$notes <- NULL
mu_sf <- mu_sf %>% 
  dplyr::mutate(cluster = as.character(ID)) %>% 
...
mu_notes <- mu_sf %>% 
  dplyr::select(-ID, -notes) %>% # remove redundant columns
  dplyr::left_join(
    dplyr::mutate(colour_scale_data, cluster = as.character(group)), 
    by = "cluster"
  )

pn <- mu_notes %>% #mutate(id = as.numeric(cl)) %>% arrange(id) %>% 
  ggplot() + 
  geom_sf(aes(fill = notes), colour = "grey70", lwd = 0.05) + 
