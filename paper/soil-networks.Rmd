...
abundance <- dplyr::left_join(select(mu_sf, gid, mukey), x) %>%
  st_as_sf %>% 
  mutate(
    area = st_area(.),
  ...
  group_by(compname) %>% 
  summarise(
    abundance = sum(area_w),
    abundance_ha = units::set_units(measurements::conv_unit(abundance, from = "m2", "hectare"), "ha"),
    do_union = FALSE
  )
```
...
## Creating graphs from soil survey data
...
In this study, we analysed the soil map produced by conventional mapping using tools from the field of network science [REFERENCE]. 
Namely, these tools were used to analyse the co-occurence of different soil series over the survey area. 

### Soil siblings

The original, tabular soil survey data was organised so that one or more soil series were associated within each map unit delineated over the survey area.   
Additionally, the relative proportion of each soil serie within each map unit was estimated by the pedologist who surveyed the area (Table \ref{tab:sim_mat_1}).

In this study, we refer to "soil siblings" as the set of soil series that co-occur in at least one map unit. 
On the subset of soil survey data provided Table \ref{tab:sim_mat_1}, soil series \textit{Amador}, \textit{Gillender}, \textit{Ranchoseco}, \textit{Vleck}, \textit{Pardee}, and \textit{Peters} are all siblings, since they co-occur in map unit "1612048". 
However, \textit{Vleck} and \textit{Miltonhills} are not siblings, at least on that subset of data, since they do not co-occur in any of the two map units presented in this Table. 
...
\label{eq:jaccard}
\end{equation}
where B is Bray-Curtis dissimilarity, defined as:
\begin{equation}
D(j,k) = \frac{\sum abs(X(i,j) - X(i, k))}{\sum (X(i, j) + X(i, k))}
\label{eq:bray}
\end{equation}
...
It is a force-directed layout algorithm, which tries to project a graph so that edges have similar length and do not cross each other. 

DION?

```{r ex_graph, echo=FALSE, fig.height=8, fig.width=13, fig.cap="\\label{fig:ex_graph}Generation of an undirected graph representation of soils urvey data. (a) Soil series represented as vertices. (b) "}
...
We used the fast greedy modularity optimisation algorithm [@clauset2004finding], which... 
...
```{r components_per_polygon, echo=FALSE, fig.cap="\\label{fig:components_per_polygon}Number of soil components mapped in the different map units in the surveyed region."}
...
pn <- mu_union %>% #mutate(id = as.numeric(cl)) %>% arrange(id) %>% 
  ggplot() + 
  geom_sf(aes(fill = notes), colour = "grey70", lwd = 0.05) + 
  scale_fill_manual(values = colour_scale_data$color_simple, guide = guide_legend(title = "")) +
  # scale_fill_manual(values = mu_union$color, guide = guide_legend(title = "")) + 
  theme_bw()
